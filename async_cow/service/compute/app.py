# -*- coding: utf-8 -*-
from async_cow.auth import QiniuMacAuth
from async_cow.service.compute.config import KIRK_HOST
...
class AccountClient(object):
    ...
    def __init__(self, cow, host=None):
        self.cow = cow
        self.qcos_clients = {}
        if self.cow.auth is None:
    ...
    async def init(self):
        ...
        # 官方SDK 在构造函数中获取了self.uri, 但并没有发现调用
        # 不明所以，先实现了再说，使用者可以不需要关心
        if self.uri is None:
            acc, info = await self.get_account_info()
            self.uri = acc.get('name')

    async def get_qcos_client(self, app_uri):
        """获得资源管理客户端
        缓存，但不是线程安全的
        """
        ...
        if client is None:
            client = await self.create_qcos_client(app_uri)
            self.qcos_clients[app_uri] = client

        return client

    async def create_qcos_client(self, app_uri):
        """创建资源管理客户端
        ...
        products = await self.get_app_region_products(app_uri)
        auth = await self.get_valid_app_auth(app_uri)
    ...
    async def get_app_keys(self, app_uri):
        """获得账号下应用的密钥
        ...
        return await self.cow.http._get_with_qiniu_mac(url, None, self.cow.auth)

    async def get_valid_app_auth(self, app_uri):
        ...
        ret, retInfo = await self.get_app_keys(app_uri)
    ...
    async def get_app_region_products(self, app_uri):
        """获得指定应用所在区域的产品信息
    ...
    async def get_region_products(self, region):
        ...
        regions, retInfo = await self.list_regions()
        if regions is None:
            return None
    ...
    async def create_app(self, args):
        """创建应用
        ...
        url = '{0}/v3/apps'.format(self.host)
        return await self.cow.http._post_with_qiniu_mac(url, args, self.cow.auth)

    async def delete_app(self, app_uri):
        """删除应用
        ...
        return await self.cow.http._delete_with_qiniu_mac(url, None, self.cow.auth)
